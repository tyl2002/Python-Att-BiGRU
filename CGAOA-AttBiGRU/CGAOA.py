...
def TGAOA(pop, dim, lb, ub, MaxIter, fun):
    ...
    all_bestx = np.zeros([MaxIter, dim])
    mean_fit = np.zeros([MaxIter, 1])
    x_1st = np.zeros([MaxIter, dim])
    Xnew = copy.copy(X)
    ...
    for t in range(MaxIter):
        ...
        Curve[t] = GbestScore
        all_bestx[t, :] = GbestPositon
        mean_fit[t] = np.mean(fitness)
        x_1st[t, :] = X[0, :]

    return GbestScore, GbestPositon, Curve, all_bestx, mean_fit, x_1st
